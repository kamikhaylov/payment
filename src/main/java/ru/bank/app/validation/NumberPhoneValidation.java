...
public class NumberPhoneValidation<T> {
    ...
    public NumberPhoneValidation checkPrefix() {
        if (!numberPhone.startsWith("+7")) {
            throw new NumberPhoneException("Неправильный формат номера, номер должен начинаться с +7 "
    ...
    public NumberPhoneValidation checkLength() {
        if (numberPhone.length() != 12) {
            throw new NumberPhoneException("Неправильный формат номера, номер должен содержать 12 символов "
                    + numberPhone, numberPhone);
        }
        return this;
    }
    ...
    public NumberPhoneValidation checkAllNumber() {
        ...
        for (int i = 0; i < numberSymbol.length(); i++) {
            if (!Character.isDigit(numberSymbol.charAt(i))) {
                throw new NumberPhoneException("Неправильный формат номера, после префикса +7 - номер должен содержать только цифры "
                        + numberPhone, numberPhone);
            }
        }
        return this;
